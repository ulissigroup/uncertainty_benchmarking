...
This could be done by training \gls{BNN} to minimize the Kullback-Leibler divergence between the variational distribution and the true posterior.
We sample the model parameters K times from the approximated posterior, and use the mean of these models’ predictions as the final prediction and the standard deviation of those predictions as the estimation of uncertainty.
We implemented \gls{BNN} and performed variational inference using Pyro.\cite{Bingham2018}
