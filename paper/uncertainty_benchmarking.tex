...
Regarding \gls{NLL}:  The \gls{dGP} and \gls{GP} methods yield the best (i.e., lowest) \gls{NLL} values.
Note how the under-confident \gls{GP} model has a worse miscalibration area and sharpness than the \gls{dNN} and \gls{PFGP} models, but a better \gls{NLL} value.
...
Given the performance metrics for accuracy, calibration, sharpness, and \gls{NLL}, we expect the \gls{dGP} method to yield the best performing \gls{UQ} model for our dataset.
It yields the lowest miscalibration area and \gls{NLL} while maintaining a relatively competitive accuracy and moderate sharpness.

When choosing \gls{UQ} methods for different applications though, other factors should be considered.
For example:  Although the \gls{dGP} method performed the best here, it required the use of a hand-crafted set of features.
...
In some cases, it may be easier to use a \gls{UQ} method that does not require any additional features beyond the \gls{NN} input, such as \gls{dNN} or \gls{PFGP}.
...
Conversely, \gls{NN} ensembles also have a higher computational training cost than some of the other methods used here, such as \gls{dNN} or \gls{PFGP}.
...
The best performing method was an \glsentryfull{dGP}, which used a \gls{NN} to make adsorption energy predictions and then used a subsequent \gls{GP} to make uncertanity estimates.
...
We purposefully ommitted recalibration in this study to (1) simplify the illustration of the assessment procedure; (2) assess the innate performance of each of these UQ methods without confounding with recalibration methods; and (3) reduce overhead investment.
