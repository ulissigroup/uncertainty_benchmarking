...
from cgcnn.dropoutmodel30 import CrystalGraphConvNet
...
class train_end_load_best_valid_loss(skorch.callbacks.base.Callback):
    def on_train_end(self, net, X, y):
        net.load_params('./histories/%i_d30_valid_best_params.pt' % k)
...
for k, (indices_train, _) in enumerate(k_folder.split(sdts_train)):
    ...
    cp = Checkpoint(monitor='valid_loss_best', fn_prefix='./histories/%i_d30_valid_best_' % k)
...
for k, (indices_train, _) in enumerate(k_folder.split(sdts_train)):
    ...
    net.initialize()
    net.load_params(f_history='./histories/%i_d30_valid_best_history.json' % k,
                    f_optimizer= './histories/%i_d30_valid_best_optimizer.pt' % k, 
                    f_params='./histories/%i_d30_valid_best_params.pt' % k)
    nets.append(net)
...
with open('histories/assess_ensemble_plots_d30.pkl', 'wb') as saveplot:
